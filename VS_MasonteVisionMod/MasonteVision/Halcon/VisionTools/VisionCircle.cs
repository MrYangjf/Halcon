...
namespace HalconVision.VisionTool
{
    ...
    public class VisionCircle : IVisionTool
    {
        ...
        public VisionCircle()
        {
            ...
            IcoImage = global::HalconVision.Properties.Resources.cross;
        }
        ...
        public override bool RunVision(HObject nImage, ViewDisplayCtrl m_hWindow_Vision)
        {
            ...
            {
                if (ROIFindCtrl.ROIList[0] is ROICircle)
                    dataroi = ((ROICircle)ROIFindCtrl.ROIList[0]).getModelData();//midR, midC, radius
                else if (ROIFindCtrl.ROIList[0] is ROICircularArc)
                    dataroi = ((ROICircularArc)ROIFindCtrl.ROIList[0]).getModelData();//midR, midC, radius, startPhi, extentPhi
                else
                    dataroi = new HTuple();
            }
            else
            ...
            if (hv_Col.Length > 0)
            {
                ...
                HOperatorSet.DispObj(ho_Circle, m_hWindow_Vision.ViewHwindow);
                HOperatorSet.DispObj(ho_CircleCenterCross, m_hWindow_Vision.ViewHwindow);
                HOperatorSet.SetColor(m_hWindow_Vision.ViewHwindow, "blue");
