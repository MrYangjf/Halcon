...
namespace MasonteVision.Halcon
{
    ...
    /// (此类允许您将函数打印到Windows.Forms.Control中
    /// 该函数必须以double、float或int值数组的形式提供
    /// 初始化类时，必须提供实际绘制函数图的控件。在初始化过程中，您还可以决定是否使用鼠标事件返回绘图函数的（x，y）坐标
    /// x-y轴的缩放是自动执行的，取决于函数的长度以及y轴缩放的设置：
    ///    * AXIS_RANGE_FIXED
    ///     * AXIS_RANGE_INCREASING
    ///     * AXIS_RANGE_ADAPTING 
    ///  函数plot类的约束：
    /// 到目前为止，只能正确地绘制包含正y值范围的函数。
    /// 也只能绘制正的和上升的x范围。坐标系的原点设置在控件的左下角。
    /// 原点的另一个定义以及坐标轴的方向尚未实现。)
    /// </summary>
