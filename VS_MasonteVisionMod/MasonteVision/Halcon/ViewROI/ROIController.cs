...
namespace MasonteVision.Halcon
{
    ...
    /// （这个类创建并管理ROI对象。它使用mousedownanction和mouseMoveAction方法响应鼠标设备输入。
    /// 当你构建自己的C#项目时，你不必详细了解这个类。但是，如果要在应用程序中使用交互式ROI，必须考虑以下几点：
    /// ROIController和HWndCtrl类之间有非常紧密的连接，这意味着您必须用HWndCtrl“注册”ROIController
    /// 以便HWndCtrl能够预先明白用户输入的事件（如鼠标事件），将其连接到ROIController类中操作。
    /// ROI对象的可视化和操作都由ROIController完成。
    /// 该类提供了一种通过ROI集的列表查找 样本region的方式给予匹配应用特殊支持。）
    /// 为此，ROI集是根据这些标志加和减的。
    /// </summary>
    public class ROIController
	{
		...
		/// <summary>Index of the active ROI object
        /// </summary>
		...
		/// <summary>List containing all created ROI objects so far
		/// </summary>
		...
		public HRegion GetIndexRegion(int index)
		{
			...
			//	}//end of switch
			//}//end of for
			tmp = ((ROI)_ROIList[index]).getRegion();
			tmpAdd = tmp.Union2(tmpAdd);
