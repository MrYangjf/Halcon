...
namespace MasonteVision.Halcon.VisionTool
{
    ...
    public class VisionLine : IVisionTool
    {
        ...
        public override bool RunVision(HObject nImage, HaGUI m_hWindow_Vision)
        {
            ...
            if (ROIFindCtrl != null)
            {
                if (ROIFindCtrl is ROILine)
                    dataroi = ((ROILine)ROIFindCtrl).getModelData();//row1, col1, row2, col2
                else if (ROIFindCtrl is ROIRectangle1)
                    dataroi = ((ROIRectangle1)ROIFindCtrl).getModelData();//
                else
                    dataroi = new HTuple();
            }
            else
            ...
            if (dataroi.Length == 4)
            {
                ...
                hv_FindRowBegin = (dataroi[0] + dataroi[2]) / 2;
                hv_FindColEnd = dataroi[3];
                hv_FindRowEnd = (dataroi[0] + dataroi[2]) / 2;
            }
